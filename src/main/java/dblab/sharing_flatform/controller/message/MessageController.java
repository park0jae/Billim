...
public class MessageController {
    ...
    public Response findSendMessage(@Valid MessagePagingCondition cond){
        cond.setSenderUsername(getCurrentUsernameCheck());
        return Response.success(messageService.findSendMessage(cond));
    }

    @ApiOperation(value = "수신 메시지 조회", notes = "현재 로그인한 유저가 수신한 메시지를 모두 조회합니다.")
    @GetMapping("/receive")
    @ResponseStatus(HttpStatus.OK)
    public Response findReceiveMessage(@Valid MessagePagingCondition cond){
        cond.setReceiverUsername(getCurrentUsernameCheck());
        return Response.success(messageService.findReceiveMessage(cond));
    }
